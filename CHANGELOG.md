...
## [0.4.1] - 修复：右键菜单第二次右键弹出浏览器原生菜单

- **问题**：行右键菜单打开后，第二次右键（无论点在空白处还是别的行）会冒出系统原生右键菜单，行为割裂。根因是菜单下方的全屏遮罩（`.popover-mask`）挡住了行，第二次右键打到遮罩上、而遮罩没有拦截 `contextmenu`，于是浏览器原生菜单接管。
- **修法**：移除行菜单的全屏遮罩，改为在菜单打开期间全局捕获 `contextmenu` / `mousedown`——任何右键都先 `preventDefault` 吃掉原生菜单；点在菜单内则保持，点在某行则**一次右键即把菜单移动到该行重新打开**，点其他位置则关闭。`mousedown` 仅响应左键，避免右键的 mousedown 先关掉菜单导致无法「右键换目标行」。行元素新增 `data-lIdx` 供全局处理器定位目标行。
